...
-- Calculate GrossAmount based on the randomly generated Quantity and Equipment's UnitPrice
SELECT @GrossAmount = UnitPrice * @Quantity FROM Equipment WHERE EquipmentID = @EquipmentID

--7. DiscountAmount - Discount amount on the equipment applies to quantities purchased i .e. this 
--   applies irrespective of the number of quantity purchased

-- Calculate DiscountAmount based on the randomly generated GrossAmount and Equipment's DiscountPercent
...
/*
IF OBJECT_ID('tempdb..#VarRate') IS NOT NULL
	DROP TABLE #VarRate
...
SELECT DISTINCT(PostalCode), 
 CASE 
	...
	WHEN PostalCode BETWEEN 70001 AND 90000 THEN 0.062
	ELSE 0.0078
END AS VariableRate INTO #VarRate
FROM Customer

SELECT * FROM #VARRATE

DEclare @CustomerID int = 1
DEclare @PostalVariationAmount float
Declare @GrossAmount int = 10

SELECT @PostalVariationAmount = @GrossAmount * (1 - (SELECT VariableRate FROM #VarRate WHERE PostalCode = (SELECT PostalCode FROM Customer WHERE CustomerID = @CustomerID)))
*/
...
-- SELECT @TransDate, @CustomerID, @EquipmentID, @Quantity, @GrossAmount, @DiscountAmount
...
SELECT @FloatRateAmount DECIMAL(12, 2)
SELECT @FloatExceededAmount DECIMAL(12, 2)
SELECT @PostalVariationAmount DECIMAL(12, 2)
...
-- Generate a random quantity between 1 and 250
SET @quantity = CAST(RAND() * 250 AS INT) + 1

-- Calculate the gross amount based on the unit price from the Equipment table
SELECT @grossamount = UnitPrice * @quantity
FROM Equipment;

-- Display the calculated values
SELECT @quantity AS Quantity, @grossamount AS GrossAmount;
		...
		SELECT @GrossAmount
			WHILE
			BEGIN
				IF OBJECT_ID('tempdb..#Quantity') IS NOT NULL
				DROP TABLE #Quantity
				SELECT ROW_NUMBER() OVER (ORDER BY EmpID) AS nEmpID, EmpID, LastName + ' ' + Firstname AS Name INTO #PharmAssist
					FROM Employee
					WHERE RoleID = 2
